...
class ServerController extends AbstractServerController {
	...
	protected function executeEdit() {
		...
		if(!empty($edit_server)) {
			...
			$tpl_data = array_merge($tpl_data, array(
				...
				'edit_glip_selected_' . $edit_server['glip'] => 'selected="selected"',
			));
		}

		$notifications = array('email', 'sms', 'pushover', 'glip');
	...
	protected function executeSave() {
		...
		$clean = array(
			...
			'glip' => in_array($_POST['glip'], array('yes', 'no')) ? $_POST['glip'] : 'no',
		);
	...
	protected function getLabels() {
		return array(
			...
			'label_glip' => psm_get_lang('servers', 'glip'),
			'label_glip_token' => psm_get_lang('servers', 'glip_token'),
